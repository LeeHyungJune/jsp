...
@WebServlet(asyncSupported = true, urlPatterns = { "/test/loginProc.pink" })
public class LoginProc extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//	여기서는 데이터베이스에 있는 비밀번호로 임시로 강제로 세팅한다.
		ppw = "12345";
		...
		/*
		 	...
		 	카운트가 0인 경우는 없는 회원이라는 뜻
		 	
		 	따라서 카운트가 1인 경우는 로그인 처리를 하고 메인페이지로 돌려보내고
		 	카운트가 0인 경우는 다시 로그인 페이지로 돌려보내야 한다.
		 	
		 	참고 )
		 		로그인 처리
		 			
		 			세션에 "SID" 라는 키값으로 아이디를 입력해놓기로 약속했다.
